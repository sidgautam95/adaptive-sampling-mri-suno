"""
Greedy sampling optimization for a single multicoil MRI scan/slice.

User specifies:
  - reconstruction model (e.g., U-Net or MoDL variant)
...
# Crop and normalize ground truth for the greedy optimization example
# (crop_img() should be defined in utils.py)
...
budget = W // us_factor
num_centre_lines = budget // 3  # initialize with 1/3rd of budget in low-frequency center

initial_mask = make_lf_mask(H, W, num_centre_lines)
...
# -------------------------------------------------------------------------
# RUN GREEDY SAMPLING OPTIMIZATION
# -------------------------------------------------------------------------

greedy_mask, loss_greedy_list = greedy_sampling_optimization(
    ksp, mps, img_gt, initial_mask, budget, model, device, nChannels,
